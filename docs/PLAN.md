...
| **6** | Data layer + crypto. Room + SQLCipher, Argon2id, envelope v2, Keystore device wrap + BiometricPrompt. **Done:** **cross-implementation test vectors** — a web-app envelope decrypts in Kotlin and vice versa, committed to *both* repos. | M | instrumented | **partial** — envelope v2 + Argon2id done (cross-repo test vectors passing both directions); Room + SQLCipher + Keystore-wrapped DB key done and verified on-device (real encryption at rest confirmed). See `docs/handoff/PHASE-06.md`. Not done: `:app` still on old `SongStorage.kt`, account-DEK Keystore/BiometricPrompt device wrap, Room migrations |

...
| **11** | Hardening + Play release. R8, baseline profile, Data Safety form, privacy policy, internal testing track, manual device matrix. | M | ✅ | not started — a pre-existing gap was found and scoped ahead of phase start: the app currently launches to `Screen.Diagnostics` (a dev-only dashboard mixing calibration-wizard buttons with real product nav) in **every** build, and Scratchpad/Piano have no real navigation entry point outside it. A fix is planned but **not yet implemented**: `Screen.Songs` becomes the default/home screen in both debug and release builds, Scratchpad/Piano/sign-in get real entry points on `SongListScreen`, and Diagnostics/the three calibration screens go behind a single `BuildConfig.DEBUG`-gated entry button (a runtime guard, not a debug-only source-set split — their compiled code still ships in the release APK until this phase's own R8 work strips the dead branch). See `docs/handoff/PHASE-11-prep-navigation.md` |

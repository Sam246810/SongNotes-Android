...
> **Phase 13 update (2026-08-15):** this section describes the *web app's*
> sync model, which is unchanged. Android diverges deliberately: sync is
> opt-in (an account is optional) and strictly manual (push/pull only on an
> explicit user action) rather than an automatic background `SyncWorker` —
> see `docs/handoff/PHASE-13-local-first.md`. There is no periodic
...
| # | Goal | Size | Device? | Status (2026-08-13) |
|---|---|---|---|---|
...
| **10** | Scratchpad product UI. Timeline, touch clip drag/trim, waveform from a C++ peak pyramid. Minimized transport strip, DAW collapsible to tempo/BPM/start-stop, theme moved into settings. | L | ✅ | **done** — timeline with playhead + tap-to-scrub punch-in, touch clip drag/trim (same-track overlaps auto-spliced via the existing `punchIn`, never left layered), waveform rendering from a C++ peak pyramid, a per-project configurable time signature (drives the metronome's actual count-in/click pattern and a new beat/bar timeline grid, not just a display), and a force-phone-mic recording toggle (`AudioEngine.setPreferredInputDevice`, new on this repo — lets the click play out to a connected headset while input stays pinned to the built-in mic), verified on a physical device with real Bluetooth headphones (Sony WF-1000XM5). Recording through a connected Bluetooth device's own mic via SCO was also built and got working end-to-end, but was **reverted** after real-device use showed the SCO route doesn't durably hold against this app's deliberately non-"communication" audio streams (the fix — `MODE_IN_COMMUNICATION` — would force AEC on and break calibration, a tradeoff `docs/PLAN.md` already rules out); Bluetooth now always forces the phone mic, with no device-mic alternative offered. USB/wired headset mic input is expected to work via the existing `setPreferredInputDevice` path (no SCO involved) but is untested — no USB headset with a mic was available. See `docs/handoff/PHASE-10.md`. Minimized transport strip / DAW-collapsible layout / theme-in-settings are UI polish with no engine dependency and were not attempted this pass. |
| **11** | Hardening + Play release. R8, baseline profile, Data Safety form, privacy policy, internal testing track, manual device matrix. **Superseded in part by Phase 13**: Data Safety/privacy-policy answers about account data are now stronger (sync is opt-in), but the account-deletion blocker this phase surfaced is unchanged. | M | ✅ | **in progress** — R8 shrinking (`isMinifyEnabled`/`isShrinkResources`, hand-written `proguard-rules.pro` covering JNI/kotlinx.serialization/WorkManager/BouncyCastle/SQLCipher) and baseline profile generation (new `:baselineprofile` module, a real profile generated and verified embedded in the release APK) are both **done and verified on a physical device** — real native recording/playback/piano, Room+SQLCipher, and Supabase auth all exercised on the R8-shrunk build with zero crashes, mapping.txt confirms every JNI/serialization-sensitive class kept its name. Privacy policy and Data Safety form are **drafted** (`docs/PRIVACY_POLICY.md`, `docs/DATA_SAFETY_FORM.md`) from the actual code, not a template — and drafting them surfaced a real pre-submission blocker: Play requires an in-app/web account-deletion path for apps with account creation, which this app doesn't have yet (sign-out only). Still open, all needing the developer's own Play Console account or physical hardware this session didn't have: real release signing (still the temporary debug-signing placeholder from Phase 11's own nav prep), building the account-deletion feature, hosting/submitting the privacy policy and Data Safety form, the internal testing track, and a multi-device matrix (everything so far verified on one physical device, a Galaxy Z Fold). A pre-existing navigation gap found while scoping this phase was already fixed ahead of the phase itself: the app used to launch to `Screen.Diagnostics` (a dev-only dashboard mixing calibration-wizard buttons with real product nav) in **every** build, with Scratchpad/Piano reachable only by routing through it. `Screen.Songs` is now the default/home screen in both debug and release builds, Scratchpad/Piano/sign-in have real entry points on `SongListScreen`, and Diagnostics/the three calibration screens sit behind a single `BuildConfig.DEBUG`-gated entry button (a runtime guard, not a debug-only source-set split — their compiled code still ships in the release APK until R8's dead-code elimination strips the branch for free). See `docs/handoff/PHASE-11-prep-navigation.md` and `docs/handoff/PHASE-11.md` |
| **12** | Forgot password + recovery-code lifecycle. Cross-platform recovery-code normalization + rewrap parity, a recovery-code unlock screen, `dek_id` stamping so a DEK rotation elsewhere is detected before syncing. **Done:** a user who forgets their account password and has their recovery code gets back in with zero data loss on both platforms; one who doesn't can start fresh without the app silently corrupting itself. | M | ✅ | **done** — full design and failure-mode analysis lives in the web repo's own session notes (this was planned and executed as one cross-repo effort, web-first); Android scope was explicitly narrower by decision: shared crypto parity + a recovery-code unlock screen, with the destructive "start fresh" path and the email-link reset flow itself staying web-only (`RecoveryUnlockScreen`/`LockedAccountScreen` link out to it) rather than building Supabase deep-link handling before the app has even shipped. `AccountKeys.kt` gained `rewrapWithNewPassphrase`/`migrateWrapIfNeeded`/`regenerateRecoveryWrap` (previously only on the web side); `RecoveryCode.kt` ports `normalizeRecoveryCode`, cross-verified against the web app's real Argon2id output via a new committed fixture (`spec/recovery-code-vectors.json`) — see `RecoveryCodeGoldenFixtureTest.kt`. `SupabaseAuthRepository` gained `recoverWithRecoveryCode`/`unlockWithPassword`/`fetchCurrentDekId`, and its create-envelope paths switched from a blind upsert to a real INSERT (throws on a `user_id` conflict instead of silently overwriting a live envelope for a different DEK — the same class of bug Phase 7 already found once, on the `signUp` side; this pass found and fixed the mirror-image gap in `signIn`'s create-if-missing branch too). `envelope_rev` now guards every `user_keys` write on both platforms. `dekId` is stamped on every pushed `songs` row and tracked in `KeySession`; `SongSyncWorker` fetches the account's live `dekId` before syncing and skips (clearing the stale session) on a mismatch, so a DEK rotated elsewhere can't get local edits re-encrypted under a key that's already dead — `SyncEngine.pull` also no longer aborts its whole pass on one undecryptable row. Two real, previously-undiscovered bugs found via on-device verification against the real Supabase project (not this suite): `signUp` threw an unhelpful raw exception (`"signUpWith(Email) succeeded but no current user"`) on any project with email confirmation enabled — which this one turned out to actually have, live, not hypothetically — now shows a proper "check your email" screen and mints the recovery code on the first post-confirmation sign-in instead; and a first-guess placeholder forgot-password URL (`songnotes.app`) turned out to already be a live, unrelated third-party product, replaced with an IANA-reserved `example.com` placeholder. Deferred, by explicit decision, not oversight: Android deep-link/App Link password reset, an Android v1-envelope reader (no v1 envelopes exist to read), server-side recovery-code hashing (would weaken the zero-knowledge property for little gain). |
| **13** | Local-first Android, opt-in manual sync. An account becomes optional; every local feature works fully offline with zero account. Sync (push + pull) only ever runs on an explicit user action — no periodic/background `WorkRequest`, deliberately rejected rather than deferred. **Done:** local songs survive a raw `force-stop` mid-edit with the last-open song reopened automatically; enabling sync on a device with existing local songs (adoption) pushes every one of them without duplicating or damaging anything already on the account; deleting a never-synced song never touches the network. | M | ✅ | **done** — full writeup in `docs/handoff/PHASE-13-local-first.md`. Five pre-existing defects (silently dropped `remoteRev` on edit, resurrected tombstones, a fake "debounce" that lost keystrokes on exit, no `SongDatabase` singleton, a conflict-vs-gone-row bug that would have duplicated every song on a DEK rotation) fixed as prerequisites, since manual-only sync has no automatic retry to paper over them. New `SyncPreferences`/`SyncStatusRepository`/`SyncController` (`:core:data`) and `SyncBanner`/`SongDialogs`/`SyncOptInScreen`/`SongDraftAutosaver`/`EditorSessionStore` (`:app`). `LockedAccountScreen` no longer gates the Songs home screen — the password is demanded only at an actual Sync press. Verified via a new JVM test suite (`SongRepositoryTest`, `SyncEngineAdoptionTest`, extended `SyncEngineTest`, `SyncPreferencesTest`, 58 tests total in `:core:data`) and on the physical Galaxy Z Fold test device. Deliberately out of scope: account deletion (unchanged Phase 11 blocker), any change to the web app, wire format, or Supabase schema. |

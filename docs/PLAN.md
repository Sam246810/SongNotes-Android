...
| **9** | Import / export / piano. PDF text extraction, share/export, 29 Salamander samples through the **same C++ mixer** so piano is recordable via the same path. | M | ✅ | **partial** — export done: "Export text" copies a formatted plain-text version to the clipboard (`formatSongAsText`, ported from the web app's `exportToText` with golden fixtures), "Export PDF" generates a real paginated PDF (`android.graphics.pdf.PdfDocument`, title + colored chord/lyric lines matching the editor's own theme) and opens Android's share sheet via `FileProvider` — verified on-device, including catching and fixing a false alarm caused by `adb shell`'s binary-unsafe pull (use `adb exec-out` for binary files, not `adb shell`). See `docs/handoff/PHASE-09.md`. **Deliberately out of scope per explicit direction:** PDF import (the web app's `pdfjs-dist`-based chord-sheet import has no Android port and isn't planned). **Not started:** piano (29 Salamander samples via the shared C++ mixer) — Android's mixer currently has no audio-file decoder or pitch-shift/resample capability at all; this is real native DSP work, not a UI feature, closer in scope to Phase 3 than to the export work just shipped |

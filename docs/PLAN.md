...
| **9** | Import / export / piano. PDF text extraction, share/export, 29 Salamander samples through the **same C++ mixer** so piano is recordable via the same path. | M | ⚠️ **verification pending** | **partial** — export done (text clipboard copy + real PDF export via share sheet, verified on-device). Piano's engine is built and compiles clean (JVM reference, C++ DSP, sample assets + decoder, RT engine wiring, JNI/Kotlin facade) but **not yet verified on a physical device** — the device was disconnected mid-phase; UI + on-device verification (cross-validate C++ against the JVM reference, confirm 0 xruns, confirm audible during a real recording) is the next step whenever the phone is back. PDF import is deliberately out of scope per explicit direction. See `docs/handoff/PHASE-09.md` |
